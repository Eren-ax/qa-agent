...
- **결론**: 경쟁사 봇은 대부분 조회+에스컬레이션 중심. Write Task는 사실상 미수행.
...
- `patterns.json` (경쟁사 봇 분석 포함: gl_bot_read_task_execution, gl_bot_write_task_execution)
- `automation_analysis.md` (4-Layer 모델: RAG / Task / Hybrid / Human)
- `03_sop/*.sop.md` (11개 SOP, 경쟁사 봇 vs ALF 비교 섹션 포함)
...
- **경쟁사 봇 baseline 대비 몇 배 나은지** (경쟁사 프레이밍)

### 핵심 차이
- 단일 score → Phase 1 / Phase 2 분리 score
- ALF 절대 수치 → 경쟁사 봇 baseline 대비 상대 수치
...
### Gap 1: 시나리오 생성에 경쟁사 봇 baseline 태깅
...
**효과**: report에서 "경쟁사 봇은 이 시나리오를 0% 해결 → ALF는 86.7% 해결 = ∞배 개선" 자동 산출
...
- 경쟁사 봇 분석 데이터: `/Users/eren/sop-agent/results/벨리에v2/02_extraction/patterns.json`
...
2. **Gap 1 (경쟁사 봇 baseline 태깅)** — 경쟁사 대비 프레이밍 자동화 ("GL은 0%, ALF는 X%")
...
- 고객사별 가변 요소: 채널 URL, sop-agent 산출물 경로, 앱 함수 리스트, 경쟁사 봇 personId
- 벨리에 외 고객사에서도 경쟁사 봇 데이터 구조 (personType=bot, personId) 동일한지 확인 필요
